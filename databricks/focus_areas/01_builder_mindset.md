...
> The original workflow was very manual. PMs and designers created Figma mockups, handed them to engineers, waited for implementation, and then reviewed the result. A lot of time was lost in handoff and UI reviews, and the design component could easily become out of date with the latest codebase.
>
...
>
> That gave me a different opportunity. Instead of only putting guardrails around AI-generated results, I wanted to challenge the workflow itself. If we can give the agent enough workflow context, component context, and spec context, maybe the designer does not need to create a separate mockup first. They can build something with the agent that is already much closer to production code.
>
...
>
> After a direction is selected, designers can still refine the UI through vibe-coding. One worth mentioned lesson we learned is that the spec has to move together with the code, which is the spec-driven development concept. So whenever an agent changes code, the spec also needs to be updated or referenced. That makes the output more maintainable. and, what's more, we replace some spec written efforts from the PM as well.
>
