...
>
> Another blocker was E2E validation. In the old monolith application, every team could see all the code and run the whole system, even if they did not fully understand other teams' logic, they can still run it. But after the split, teams needed a way to validate their widgets without depending on the full source code of every other module. So we built a simulator to help teams test and verify their widgets in isolation.
>
...
>
> I thought about it carefully and pushed back. My point was that we should not take an emergency communication pattern from the past and turn it into a blocker for a new AI solution. Chat groups are messy by nature. And we would definitely don't wanna maintian thousands of chat group as we growed. If we spend too much effort trying to make the AI understand every scattered message perfectly, we may be optimizing the wrong workflow in the wrong direction.
>
