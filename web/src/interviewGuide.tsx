...
export const guidePages: GuidePage[] = [
  {
    ...
    points: [
      ...
      { cue: 'Backend 基礎', text: <>Earlier in my career, I worked heavily on backend and platform engineering. I maintained <mark className="metric">around ten microservices and hundreds of APIs</mark>, rebuilt services around authentication, db access, Redis Cache, and handled Kubernetes deployments and production incidents.</> },
  ...
  },
  {
    ...
    points: [
      ...
      { cue: '我擅長什麼', text: <>I also know that <mark className="concept">communication and planning ahead</mark> are two of my strengths. I’m good at getting engineers, PMs, and designers on the same page, turning unclear requirements into a concrete plan.</> },
      { cue: '把問題說清楚', text: <>A lot of that communication skill came from presenting architecture proposals and project updates. I learned that good communication is <mark className="concept">making the problem clear</mark>: what is happening, why it matters, what trade-offs we have, and what decision we need to make. That helps people with very different backgrounds move in the same direction.</> },
  ...
  },
  {
    ...
    points: [
      { cue: '我想要更大的舞台', text: <>My current role has given me a great opportunity to create real impact in healthcare. I’ve worked with different roles in the department to change how we build products, and I’m grateful for that. At this point, I want to take that same way of working to <mark className="outcome">a broader range of problems and industries</mark>.</> },
  ...
  },
  {
    ...
    points: [
      { cue: '一句話開場', text: <>One project I’m particularly proud of is an <mark className="concept">AI-native product development system</mark> I designed and built. Let's called them xCraft and xDesign.</> },
      ...
      { cue: '先看現成方案', text: <>When we started exploring how AI could support the design process, we first evaluated existing tools such as vercel v0, Figma Make, and Claude Design.</> },
      { cue: '現成方案的 gap', text: <>They were useful for rapid prototyping, but when we tested them for our use case, each left a gap between the design output and our real product. v0 worked well for an independent website in its own sandbox, but it was difficult to give it all the rules and context of our product to make it follow. Figma Make could use a component set in figma and maintain visual consistency across all desgins, but engineers still had to translate the design into our production code using own library. Claude Design could pass its output to Claude Code, but that still introduced another translation step back into our local component library. <mark className="concept">Every additional translation increased the risk of inconsistency</mark>.</> },
      { cue: '先嘗試沿用 xCraft', text: <>Because of those gaps, my first idea was to let designers use xCraft directly. It already worked on top of our real framework and component library, so designers could create something that was much closer to production code instead of giving engineers a separate prototype to rebuild.</> },
      { cue: '但 workflow 不一樣', text: <>But after working with our designers, I realized that simply giving them the existing xCraft workflow was not enough. xCraft was designed to work for hours and return a nearly finished feature. Designers needed something different: <mark className="concept">a lighter and faster workflow</mark> that could generate rough options, let them compare directions, and support quick iterations.</> },
      { cue: '做成設計專用版', text: <>So I did not rebuild xCraft from scratch. I extracted the reusable skills from the original workflow, added a few design-specific skills, and recomposed them into a lighter workflow optimized for rapid iteration. Designers could quickly generate several options, compare the rendered results, and refine the direction they selected. Because it still worked directly on our real framework and component library, <mark className="outcome">the result was already product code</mark>.</> },
      ...
      { cue: 'xDesign 實際成果', text: <>At the moment, xDesign has completed <mark className="metric">around 15 to 20 design tasks that our designers reviewed and approved</mark>. We started with common design patterns that our team already knew well. For those tasks, our designers estimate it reduced their manual work by <mark className="metric">roughly 90%</mark>.</>, note: '90% 只限目前 15–20 個已 review 的 common design tasks。' },
  ...
  },
  {
    ...
    points: [
      ...
      { cue: '產品已到極限', text: <>Our outpatient product had grown into a very large frontend that used across many areas of healthcare and many workflows. As we kept adding features to the same app, a change in one area often broke something unrelated. Builds and deployments took longer, and new engineers needed more time to understand the code.</> },
      { cue: '真正的 platform 問題', text: <>More importantly, this could not continue if we wanted the product to become a platform that serve thousands of hospitals. If every internal team, hospital developer, or outside partner had to understand and change one huge application just to add a feature, <mark className="concept">the developer community could never grow</mark>. The learning curve for contributing was simply too steep.</> },
      { cue: '兩種 coupling', text: <>We had to separate two things: how features shared data, and how their code was built and deployed. Fixing only one would not let teams work independently.</> },
      { cue: '先建立 xDS', text: <>Before xDS, each feature fetched and managed its own data, and features used direct watchers to keep each other updated. This was risky in healthcare because every action and every screen had to use the correct patient’s data. So we first built <mark className="concept">xDS as one shared data and patient-context layer</mark>.</> },
      { cue: '接著建立 add-on modules', text: <>Once xDS solved the shared-data problem, we moved to the second problem: separating code and deployment. That was the start of the add-on module framework. Instead of changing everything at once, we moved step by step. We tested the idea with brand-new features first, including features running outside the main page. Then we turned the main app into a shell, used Module Federation to load features inside it, and chose a few existing features with clear boundaries for the first migration.</> },
      { cue: '平台吸收複雜度', text: <>The goal was to make modules easy to build. Developers should focus on what the feature does. The platform should handle how each module is connected, loaded, updated, and deployed.</> },
      { cue: '處理醫院差異', text: <>Later, we used the same platform idea for differences between hospitals. The same business function may need a different implementation at each hospital, so we created stable Action contracts. The common product calls the same interface, and deployment settings choose the right implementation <mark className="outcome">without adding customer-specific branches to the product core</mark>.</> },
      { cue: '補回整合測試', text: <>Independent development created another problem: developers could no longer easily see how a change affected other modules. We built xEmulator, a test version of the main app where they could test a widget with realistic shared data and cross-widget behavior, without copying and changing the whole main application.</> },
      { cue: '最難是 adoption', text: <>The hardest part was not the architecture. It was getting people to adopt it. Engineering teams and PMs had to pause some feature work and spend time on the migration before they could see its full value.</> },
      { cue: '先證明再爭取', text: <>I first built small working examples to show that the design could work. I used those results to explain the value to senior leaders and the engineering and PM managers. They eventually agreed to give the migration the time it needed.</> },
      { cue: 'Ownership 分散出去', text: <>Once the direction was approved, each feature owner decided how to split their own part of the product, and I reviewed the plans to keep the platform boundaries consistent. This let the people who knew each area best lead their own migration <mark className="action">without turning the platform team into the bottleneck</mark>.</> },
      ...
      { cue: '現在的 scale', text: <>The platform has now grown to <mark className="metric">300+ independently deployable widgets and modules across about 20 engineers</mark>. The impact continues to increase as the product grows because teams can develop, deploy, and maintain their features more independently.</> },
      { cue: 'AI 延伸', text: <>One later benefit was that the clear boundaries we built for people and outside partners also made AI coding agents much easier to control. The platform kept creating new value after the original migration.</> },
      { cue: '政府認可', text: <>This design also led to <mark className="outcome">two patents granted by the Taiwan Intellectual Property Office</mark>. This was outside recognition that our architecture was a new and valuable idea.</> },
      { cue: '收尾', text: <>My biggest lesson is that a platform is more than breaking a system into modules. It succeeds when you <mark className="outcome">make it easier for each new person to contribute</mark>, while the platform still controls shared contracts, safety, and how modules are updated.</> },
    ],
  ...
  },
  {
    ...
    points: [
      ...
      { cue: 'Scale 後出問題', text: <>GraphQL worked with small examples. But our union combined data from several APIs, so large patient histories produced a much bigger and more complex response than a single API would. Parsing and checking that whole response cost more time than we saved by reducing API calls.</> },
      ...
      { cue: 'Turning point', text: <>Raw JSON was the turning point. Once we bypassed schema validation, we had removed the main reason we chose GraphQL, while still keeping the complexity and performance cost of the GraphQL layer. At that point, <mark className="concept">GraphQL was no longer giving us enough value over simpler REST APIs</mark>.</> },
