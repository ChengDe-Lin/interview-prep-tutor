...
export const appleGuidePages: AppleGuidePage[] = [
  {
    ...
    points: [
      ...
      { cue: 'Backend 與 production 基礎', text: <>Earlier in my career, I maintained around ten backend services and hundreds of APIs using Kubernetes, Redis, RabbitMQ, and databases. During my first two years, I also had <mark className="concept">formal on-call responsibility</mark> for these production services. I responded to SLA issues, service crashes, and P0 problems involving clinical data. This taught me that reliability is part of the product—especially in healthcare, where doctors and nurses depend on the system every day.</> },
      { cue: '為何轉向 platform', text: <>Later, I started taking on more platform work because I wanted to grow toward architecture. I did not want to spend all my time building one feature after another. I wanted to <mark className="concept">design how the whole system should work</mark>, set the technical direction, and build foundations that other engineers could use.</> },
      { cue: 'Add-on Framework', text: <>That led to one of my biggest projects: the Add-on Module Framework. Our healthcare product had become large and tightly coupled, so even a small change in one feature could break another feature. I led the design and migration, creating clear boundaries so teams could build and deploy features more independently while the platform still controlled shared contracts and patient context. The goal was to <mark className="concept">reduce hidden dependencies and limit the impact of each change</mark>.</> },
      ...
      { cue: 'xDS 與 healthcare safety', text: <>I also helped design xDS, a shared patient-context and clinical-data layer. Because every feature needed to use the correct patient and encounter data, <mark className="concept">keeping that data consistent was directly connected to safety</mark>. That design improved the reliability of the whole platform and became part of a patent granted by the Taiwan Intellectual Property Office.</> },
      { cue: '最近的 automation', text: <>More recently, I built xCraft and xDesign, an AI-assisted workflow that connects design and coding agents directly to our product framework. Faster development was useful, but because this is healthcare software, <mark className="concept">we cared much more about making the result complete, correct, and controllable</mark>. I added requirement tracking, tests, limited permissions, visible execution state, and clear stop conditions to support that goal.</> },
      { cue: 'Optional · 溝通與 AI adoption', text: <>I also care about helping other people adopt new technology, not only building it myself. I have led more than ten GenAI seminars for hundreds of colleagues, and I regularly explain technical direction, trade-offs, and project progress to senior leaders, including our General Manager and the company’s Vice Chairman. This has taught me how to <mark className="concept">explain complex ideas clearly and get different groups aligned</mark>.</>, note: '時間夠再講；不用在 About Me 展開 seminar 或高層簡報細節。' },
  ...
  },
  {
    ...
    purpose: '沿用 recruiter 版主軸：real impact → user trust → reliability foundation，再用真實經驗證明 fit。',
    focus: ['Real impact', 'Reliability', 'Role fit'],
    points: [
      { cue: '我想要的 impact', text: <>One thing I care about most is whether my work can create real impact. But before a product can serve a large number of users, people need to trust that it will work. That is why I see <mark className="concept">reliability as one of the foundations of the whole company</mark>.</> },
      { cue: '為什麼是這個 team', text: <>That is what attracts me to this team. It builds software, automation, monitoring, and tools that keep Apple Silicon infrastructure reliable at a very large scale. Users may never see this work directly, but it supports the services and experiences they depend on every day.</> },
      { cue: '我的 reliability 經驗', text: <>My own experience has trained me to think this way. In healthcare, a data problem, a failed deployment, or a small software mistake can affect a real clinical workflow. I learned to care about validation, understand how a system can fail, and take responsibility for what happens after the code is deployed.</> },
      { cue: 'Platform 的直接連結', text: <>Later, I started solving repeated reliability problems through platform design. With the Add-on Framework and xDS, I reduced hidden dependencies, made changes easier to isolate, and kept patient data consistent across independently deployed features.</> },
      { cue: 'Automation 的直接連結', text: <>On the AI side, I apply the same thinking to automation. I do not only ask whether the agent can finish a task. I build controls around it so the result is complete, observable, and safe to review. The technology is different, but the mindset is the same: <mark className="outcome">automate the work without giving up reliability</mark>.</> },
      { cue: '結尾', text: <>So this role connects strongly with both my experience and what motivates me. I can bring platform architecture, automation, and production judgment to a team that supports Apple at a much larger scale. <mark className="outcome">I would find that kind of work very meaningful.</mark></> },
    ],
    reminders: ['主軸是 impact 需要 trust，而 trust 的基礎是 reliability。', '具體故事只點到為止；HM3、HM4 再 deep dive。', '這題不主動講 physical-fleet 或 Kubernetes gap；被直接問才回答。', '不要提 Apple team 有 on-call。Recruiter 已說明這個 team 不需要。'],
  },
  {
    id: 'hm-addon-deep', section: 'hm', nav: 'HM · Add-on Core Story', step: 'HM3', title: 'Core story: Building a safer platform', duration: 'About 4 min',
    purpose: '這一頁才是可以從頭講到尾的主故事。只保留 problem → ownership → architecture → migration → result。',
    focus: ['Primary story', 'Architecture', 'Safe change'],
    points: [
      { cue: '開場與問題', text: <>The project that best shows how I think about reliability is our Add-on Module Framework. Our outpatient product had grown into <mark className="concept">a large, tightly coupled frontend</mark>. A change in one area could break something unrelated, releases took more than an hour, and engineers needed a lot of context before they could make a safe change.</> },
      { cue: '為何是 reliability 問題', text: <>This was not only a developer-productivity problem. In one real case, a change in a prescription-copying workflow <mark className="outcome">nearly caused medication from the previous patient to be used for the current patient</mark>. That showed us that, in healthcare, <mark className="concept">a hidden dependency can become a patient-safety risk</mark>.</> },
      { cue: '我的 ownership 與判斷', text: <>This was <mark className="action">the first large architecture problem I owned</mark> after joining the platform team. Before choosing a technology, I separated the problem into two parts. One was data coupling: every feature needed the same patient context. The other was code and deployment coupling: features were still built and released together. <mark className="concept">Splitting only the UI would not make the product safer.</mark></> },
      { cue: '先建立 data foundation', text: <>So we first built xDS as one shared patient-context and clinical-data layer. Widgets used the same patient and encounter context instead of fetching and combining data in different ways. This gave independent features <mark className="outcome">one safe and consistent view of the patient</mark>.</> },
      { cue: '再拆 code 與 deployment', text: <>Once that data foundation was in place, <mark className="action">I designed the Add-on Module Framework</mark>. I chose Module Federation because separately built features still needed to appear inside the same product. The main application became a shell that could load modules while <mark className="concept">each team owned its own build and release</mark>.</> },
      { cue: '補回 integration confidence', text: <>Independent development created a new risk: developers could no longer see the whole application locally. We built xEmulator, a test version of the shell where a team could run its widget with <mark className="concept">realistic shared data and cross-widget behavior</mark> before release.</> },
      { cue: '逐步 migration', text: <><mark className="concept">We did not migrate everything at once.</mark> We tested the idea with new features first, then selected existing features with clear boundaries. Each feature owner planned how to separate their own area, while <mark className="action">I reviewed the plans and difficult cases</mark> to keep the architecture consistent.</> },
      { cue: '推動 adoption', text: <>The hardest part was getting the organization to accept the transition cost. Teams and PMs had to slow some roadmap work before they could see the long-term benefit. <mark className="action">I built working pilots first</mark>, so we could discuss real results instead of only debating an architecture proposal.</> },
      { cue: '量化結果', text: <>During the migration quarter, even while engineers were learning the framework, <mark className="metric">delivery speed improved by 46%</mark>. More importantly, <mark className="metric">defect age decreased by 58%</mark>. The platform has since grown to <mark className="outcome">more than 300 independently deployable widgets with contributions from over fifteen engineers</mark>.</> },
      { cue: 'Apple relevance', text: <>The scale and domain are different, but the lesson transfers directly to infrastructure: <mark className="outcome">define clear ownership, roll out in stages, validate the real system, and keep one failure from spreading</mark>.</> },
    ],
    reminders: ['這一頁可以順著講；多版本、rollback、技術選型細節全部留到 HM3Q。', 'Ditto 是 near miss，不能說真的傷害病患。', '46% / 58% 是 Add-on migration quarter 的 Jira review。', 'xDS 與 xEmulator patents 已 granted；Add-on patent application 仍 pending。'],
  },
  {
    id: 'hm-addon-qa', section: 'hm', nav: 'HM · Add-on Follow-ups', step: 'HM3Q', title: 'Add-on and xDS follow-up answers', duration: 'Only answer the question asked',
    purpose: '這頁不能順著念。先聽 HM 問什麼，再跳到對應卡片回答。',
    focus: ['Follow-up only', 'Architecture details'],
    points: [
      { cue: '為什麼先做 xDS？', text: <>Independent modules would still need the same patient data. If every widget fetched and combined that data in its own way, we would only move the coupling instead of removing it. So xDS became the shared data and patient-context foundation before we separated the feature code.</> },
      { cue: 'xDS 是 service 嗎？', text: <>No. xDS is a package installed inside the product shell. Widgets use a socket client to send events to xDS in the shell, and xDS keeps and updates the data-node trees used by those widgets.</> },
      { cue: '多版本怎麼共存？', text: <>A version-two shell can keep several major-version data trees at the same time. Each widget uses the client version it was built with and communicates with the matching tree. Updates still go through the backend, which remains the single source of truth, so version-one and version-two widgets can stay synchronized.</> },
      { cue: '怎麼控制多版本成本？', text: <>We agreed to support at most <mark className="metric">three major versions</mark>, because more versions would create too much memory and maintenance cost. A data node is created only when a widget actually uses it. xDS does not fetch or watch an unused node or an unused version.</> },
      { cue: '怎麼驗證新舊資料？', text: <>During testing, we ran a script that compared the old and new data-node trees and listed every field that differed. We reviewed whether each difference was expected, came from a known breaking change, or showed an unrelated change. We used focused checks instead of logging every difference continuously because the data volume was too large.</> },
      { cue: 'Widget 怎麼 rollback？', text: <>Inside a version-two shell, we can change a widget’s deployment configuration so it loads the previous remote-module version and uses the older client again. In most cases, we can roll back that widget without rolling back the shared shell, then deploy a patched shell later if needed.</> },
      { cue: 'Production 驗證到哪裡？', text: <>The compatibility design is running across <mark className="metric">five healthcare sites</mark>. Some sites use version one, while sites that started with a version-two shell can still run version-one widgets. We have not yet upgraded an existing hospital from a version-one shell to version two in place. What production has validated is mixed-version widgets inside the version-two environment, with no production incident caused by that coexistence.</> },
      { cue: '為什麼 Module Federation？', text: <>We needed separately built features that could still appear inside the same product shell. Module Federation gave us that runtime composition model without forcing every feature back into one build and release.</> },
      { cue: '為什麼不用 iframe？', text: <>We considered iframe-based isolation. It is useful for strongly isolated workflows, and we used a separate-window approach for some off-page widgets. But for features inside the main workflow, we also needed shared user experience, patient context, and easier integration, so we did not want every feature to become an isolated page.</> },
      { cue: 'xEmulator 解決什麼？', text: <>After teams began developing independently, a widget developer could no longer see the whole application locally. xEmulator gave them a test version of the shell with realistic shared data and cross-widget behavior, so they could test integration without cloning and changing the real host application.</> },
      { cue: '最難的部分？', text: <>The hardest part was adoption, not Module Federation. Migration required engineering time before every team could see the benefit. I built working pilots, worked with management and PMs to secure migration capacity, and let each feature owner plan their own area while I reviewed the architecture boundaries.</> },
    ],
    reminders: ['每次只答一到兩張，不要從頭念到尾。', '沒有被問 multi-version，就不要主動展開。', 'xDS production 邊界：尚未做既有醫院 v1 shell → v2 shell 的 in-place upgrade。', 'Add-on patent pending；不要說成已 granted。'],
  },
  {
    ...
    points: [
      ...
      { cue: 'Requirement Atomizer', text: <>One component I built is the Requirement Atomizer. First, <mark className="concept">a script splits the requirement document by its Markdown structure</mark>. Then a dedicated agent breaks each section into small requirement items and gives each one a unique tag. During implementation, xCraft references those tags in the relevant code, and a deterministic script checks whether any tag is missing. So we do not rely on the agent to judge whether its own work is complete.</> },
      ...
      { cue: '先看成熟工具', text: <>Before building our own solution for design, we evaluated mature tools from well-known companies, including v0, Figma Make, and Claude Design. <mark className="concept">We tested them against our real use case</mark> instead of only comparing their feature lists.</> },
      { cue: '現成工具的 gap', text: <>v0 worked well for an independent webpage in its own sandbox, but it was difficult to give it all the rules of our product. Figma Make could use a component library and keep the style consistent, but the output was still made of Figma components, so an engineer had to translate it into our source code. Claude Design could pass its output to Claude Code, but bringing that output from its cloud sandbox into our local component library added another translation step. <mark className="concept">Every extra translation step added more risk.</mark></> },
      { cue: '第一個 assumption 錯了', text: <>My first idea was to let designers send work to the normal xCraft workflow. That was wrong. Engineers could wait a long time for one nearly finished result, but designers needed rough versions they could compare and change quickly.</> },
      ...
      { cue: '下一個 bottleneck', text: <>That lighter workflow removed the handoff between design and engineering, but designers still had to drive every small UI change themselves. So the bottleneck was still there. To solve that, <mark className="action">I built xDesign to handle the repeatable part of the design workflow</mark>.</> },
      { cue: 'xDesign 怎麼做', text: <>xDesign asks xCraft to build several real versions, reviews the rendered results, and sends a focused refinement task. It can continue this loop while the designers keep control of the important choices.</> },
      { cue: '一個重要 lesson', text: <>There is one lesson I learned that I would like to share: <mark className="concept">I should have separated the responsibilities much earlier</mark>. At first, xCraft was responsible for the whole feature, so it could change both the UI and the business logic. When xDesign later became responsible for the visual result, I did not change that boundary right away. The main xCraft workflow could still change the UI while adding business logic, so the final result might no longer match the design we had already reviewed.</> },
      { cue: '用 architecture 隔離責任', text: <>So I rewrote the architecture. xDesign and the main xCraft workflow could access <mark className="concept">different sets of files</mark>, and each side could edit only the files it owned. The boundary was enforced by the system, not only by a prompt.</> },
      { cue: 'Stop conditions', text: <>Once the two agents could work together, I also needed to decide when the loop should stop. If they cannot agree, go over the iteration limit, or have low confidence, <mark className="outcome">the workflow stops instead of guessing and asks a person to make the decision</mark>.</> },
      { cue: '目前結果', text: <>With those boundaries in place, xDesign has completed around <mark className="metric">15 to 20 design tasks</mark> that our designers reviewed and approved. For those common and well-understood tasks, they estimate roughly 90% less manual work.</> },
      { cue: 'Apple relevance', text: <>This project changed how I think about automation. The goal is not only to make it fast or autonomous. It also needs limited permissions, visible state, validation, stop conditions, and a clear path to human review when the system is uncertain.</> },
    ],
  ...
  {
    id: 'hm-production-evidence', section: 'hm', nav: 'HM · Production Core Story', step: 'HM5', title: 'Core story: A cache failure we did not see early enough', duration: 'About 3–4 min',
    purpose: '這一頁可以順著講完。用一個完整 incident 證明 response、root cause、permanent fix 和 observability thinking。',
    focus: ['Production incident', 'Observability', 'Learning'],
    points: [
      { cue: '故事開場', text: <>One production incident that taught me a lot happened during my first two years, when I was responsible for several backend services. It involved <mark className="concept">prebuilt cache data used by our order and drug search workflows</mark>.</> },
      { cue: '為什麼需要 cache', text: <>The search results needed data from many database tables and even from a third-party API, so building them during every user request was too slow. We ran a scheduled job during a lower-traffic period and stored the prepared results in a cache.</> },
      { cue: '原本的 TTL 設計', text: <>At that time, the refresh job updated only the records returned by its query. If a record had been deleted, it was no longer returned, so its old cache entry could remain searchable. We used a relatively short TTL to make those old entries disappear.</> },
      { cue: 'Failure 發生', text: <>The incident started when that third-party API kept failing. The cache build could not finish, but the TTL kept running. Eventually, required cache data became unavailable, and <mark className="outcome">user operations started timing out</mark>.</> },
      { cue: '為什麼太晚才發現', text: <>The impact was larger because our monitoring was incomplete. Our alerts focused on API errors and response time, but <mark className="concept">we did not monitor whether the cache had been updated successfully</mark>. We saw the strongest signal only after users were already affected.</> },
      { cue: 'Immediate mitigation', text: <>We immediately contacted the third-party vendor and worked with them to restore the API. Because we had not kept an older cache version that we could switch back to, we also deployed a temporary change that increased the request timeout. <mark className="concept">That reduced the impact while the API was being fixed, but it was only a mitigation.</mark></> },
      { cue: '重新檢查真正問題', text: <>After the incident, we looked again at why the TTL existed. The real problem was not that every cache entry needed to expire. It was that the refresh job did not know which old records had been deleted.</> },
      { cue: 'Permanent fix', text: <>We changed the job so it first listed the existing cache entries, then completed the source query and updated the current results. Only after a successful query did it delete entries that were no longer present. <mark className="outcome">Deletion no longer depended on the TTL</mark>, so we could remove it.</> },
      { cue: '補上 observability', text: <>I also added the last successful refresh time and better logs around the external API. I defined a query that checked whether the last update was older than the allowed age, and DevOps connected that condition to the production alerting pipeline.</> },
      { cue: '故事結論', text: <>The biggest lesson for me was that a healthy API does not mean the whole data path is healthy. When users depend on prepared data, <mark className="concept">data freshness is also a reliability signal</mark>. We need to monitor it before users discover the failure for us.</> },
    ],
    reminders: ['這一頁可以順著講完，不要插入 Electron 或 Kubernetes。', 'Timeout change 是 mitigation，不是 permanent fix。', '不要猜原本 TTL 時間或 cache refresh 頻率。', 'Alert condition 由你定義；DevOps 負責接上 production alerting pipeline。'],
  },
  {
    id: 'hm-production-qa', section: 'hm', nav: 'HM · Production Follow-ups', step: 'HM5Q', title: 'Production experience follow-up answers', duration: 'Only answer the question asked',
    purpose: '這頁不能順著念。Production scope、diagnosis、Electron 和 Kubernetes 是不同追問。',
    focus: ['Follow-up only', 'Honest boundaries'],
    points: [
      { cue: '你的 production responsibility？', text: <>During my first two years, I had formal on-call responsibility for around ten backend services. I handled SLA failures, service crashes, and P0 cases where clinical data was missing or not updated correctly.</> },
      { cue: '平常怎麼查問題？', text: <>We kept the most important logs in Azure Application Insights. I would start there, find the correlation ID, retrieve the detailed logs from disk storage, and reconstruct the request path before deciding what to change.</> },
      { cue: 'Cache incident 你負責什麼？', text: <>I was the engineer responsible for handling the incident and the follow-up changes. I added the cache-update information and external-dependency logs, defined the stale-cache query and alert condition, and worked with DevOps to connect it to the production alerting system.</> },
      { cue: '現在回看還會改什麼？', text: <>Looking back, I would keep a complete new cache snapshot separate from the active one, validate it, and switch to it only after the build succeeds. That would give us a safer fallback and make partial updates easier to control.</> },
      { cue: '另一個 mitigation example？', text: <>In an Electron cache case, Chromium sometimes reused an old API response after a local service restarted. Cache clearing was not reliable, so I introduced rotating ports to create a new origin and reduce the immediate user impact. I treated that as mitigation, not a confirmed root-cause fix.</> },
      { cue: '現在的 production risk？', text: <>My current work is on shared platform infrastructure. I see fewer feature-level incidents, but a defect in the shared layer can affect many applications. That makes staging checks, regression coverage, compatibility, and safe rollout part of my normal engineering decisions.</> },
      { cue: 'Kubernetes 做過什麼？', text: <>Around two to three years ago, the development team owned the Kubernetes Deployment, Service, and Ingress definitions for our backend services. Environment variables were managed through a separate configuration repository. After a change was merged, CI/CD deployed it automatically to development and staging for validation.</> },
      { cue: 'Kubernetes 的 boundary？', text: <>DevOps owned the clusters and the final production deployment. My experience was with application manifests, automated development and staging deployment, and pre-production validation. It was real hands-on experience, but it was not production cluster operation and it is not my current day-to-day work.</> },
    ],
    reminders: ['每次只回答被問到的卡，不要整頁念。', '這個 Apple team 沒有 on-call；production 經驗用來證明 judgment，不是說 team 工作內容。', 'Electron rotating ports 只稱 mitigation。', '沒有清楚記得的 Kubernetes incident 就不要補。'],
  },
  {
    ...
    points: [
      { cue: '1 · 最希望新人帶來什麼', text: <>When you decided to hire for this role, <mark className="concept">what problem did you most want this person to help the team solve, or what expertise did you hope they would bring?</mark></> },
      { cue: '2 · 成功的人', text: <>When you think about engineers who have been especially successful on this team, <mark className="concept">what do they do differently?</mark></> },
      { cue: '3 · 後續面試重點', text: <>Could you share <mark className="concept">what the following interviews will focus on and which skills the team will evaluate most closely</mark>?</> },
    ],
    reminders: ['依序問這三題即可。', '第 3 題問 evaluation areas，不要追問具體題目。', '如果時間不夠，優先問第 1、2 題。'],
  },
  {
    ...
    points: [
      ...
      { cue: '為何走向 platform', text: <>Later, I moved into platform architecture because our large, tightly coupled product was creating production risk. A local change could break an unrelated workflow, and releases were becoming slower and harder to control.</> },
  ...
  },
  {
    ...
    points: [
      { cue: '完整背景', text: <>We had three clinical applications in separate repositories, but each application was still tightly coupled internally. Releases took more than an hour, and engineers could not trace every hidden dependency. A prescription-copying change once nearly reused medication from the previous patient, so this was <mark className="concept">a patient-safety problem</mark>, not only technical debt.</> },
