...
function initialView(): View {
  ...
  const id = route.startsWith('databricks/') ? route.slice('databricks/'.length) : ''
  return pageById.has(id) ? id : 'home'
}

export default function DatabricksGuide() {
  ...
  function navigate(id: View) {
    setView(id)
    setSidebarOpen(false)
    window.location.hash = id === 'home' ? 'databricks' : `databricks/${id}`
    document.querySelector('.content-scroll')?.scrollTo({ top: 0 })
  }
  ...
  return <div className="fde-guide app-shell">
    ...
    <main className="workspace">
      <div className="topbar"><div className="guide-links"><button className="all-prep-link" onClick={() => { window.location.hash = '' }}>← 全部公司</button><button className="back-home" onClick={() => navigate('home')}>Databricks 面試流程</button></div><div className="shortcuts"><span><kbd>K</kbd> 上一頁</span><span><kbd>J</kbd> 下一頁</span></div></div>
